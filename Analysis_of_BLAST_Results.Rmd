...
## Computational
  ...
  Computation with this data began with creating a bashscript with code which downloaded the list of files (from the NCBI Sequence Read Archive study number ERP022657) in the run table to the project's data directory. Code was then written to create QC reports for each of the runs using the FastQC programan and then trimmed the sequences based on their quality scores (any sequences below the length of 150 base pairs and any base score below 25 was discarded). Then the bascript converted fastq files into fasta files so that they could be used in BLAST queries. Finally, each sequence was blasted with the result output into a csv file. These BLAST results are what are now being used for this R-scripted analysis. 
